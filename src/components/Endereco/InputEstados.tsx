...
type props = {
    cep: string
    estadoAtual: string
    setEstadoAtual: React.Dispatch<React.SetStateAction<string>>
}

export default function ({cep, estadoAtual, setEstadoAtual}: props) {
    ...
    const [listado, setListado] = useState(false)
        ...
        async function selecionarEstado() {
            ...
            const estado = option.find(el => el.value === estadoAtual)
            if (estado) estado.selected = true
        }

        async function buscarEstadoSemCep(el: ChangeEvent<HTMLSelectElement>) {
            setListado(true)
            setEstadoAtual(el.target.value)
        }
